...
In order to communicate with each other, we plan to use our .wisc emails and an Outlook group to communicate asynchonously about project details. We also created a github repository with each group member having collaborator access so that we can edit our R Markdown and HTML files seperately; this means that it is less necessary to schedule physical meet ups which is difficult with everyone's schedules. 
